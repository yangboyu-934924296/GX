...
//饼图
function pie_chart(chart,data,tit) {
	var color = ["#125774", "#26d3e3", "#265ebb", "#31be52", "#bfbe3b", ]
	if(echarts.getInstanceByDom(chart)){
		var mychart = echarts.getInstanceByDom(chart);
		mychart.setOption({
			series:[{name:"占比",data:data}]
		})
	}else{
		var mychart = echarts.init(chart);
		var option = {
...
//端口开闭情况
function port_chart(chart,data,tit){
	if(data.xdata==""||data.xdata==undefined){
		data.xdata = []
	}
	...
	var option = {
		color:["#1beb90","#f4ce1d","#00f0ff","#8f12eb","#12eb69", "#fffc12", "#00ffde", "#ff3a3a"],
		legend: { //图例
			top: "top",
			right: "center",
			textStyle:{
		...
		tooltip:{
			...
			formatter:function (data) {
				...
				for(var i =0 ;i<data.length;i++){
					str= str+data[i].seriesName+":"+data[i].value+"<br/>"
				}
			    return str;
			}
		},
...
//饼环图
function craete_pie(chart, patch, title) {
	var pie = []
...
//线
function create_line(chart,data,color){
	var series = []
	for (var i = 0; i < data.data.length; i++) {
		var item = {
			name: data.data[i].name,
...
var attack = {  GB:{name:"英国",url:"GB",size:[132/3,219/3]},
				NL:{name:"荷兰",url:"NL",size:[164/3,195/3]},
				US:{name:"美国",url:"US",size:[213/3,164/3]},
				CN:{name:"中国",url:"CN",size:[241/3,199/3]},
				RU:{name:"俄罗斯",url:"RU",size:[216/3,129/3]},
				NL:{name:"荷兰",url:"NL",size:[164/3,195/3]},
				JP:{name:"日本",url:"JP",size:[298/3,199/3]},
				BE:{name:"比利时",url:"BE",size:[189/3,153/3]},
				BG:{name:"保加利亚",url:"BG",size:[236/3,158/3]},
				CL:{name:"智利",url:"CL",size:[141/3,190/3]},
				VN:{name:"越南",url:"VN",size:[112/3,239/3]},
				DE:{name:"德国",url:"DE",size:[147/3,210/3]},
				KR:{name:"韩国",url:"KR",size:[142/3,228/3]},
				SG:{name:"新加坡",url:"SG",size:[293/3,174/3]},
				CA:{name:"加拿大",url:"CA",size:[181/3,146/3]},
				SE:{name:"瑞典",url:"SE",size:[118/3,275/3]},
			}
var grounp = {
